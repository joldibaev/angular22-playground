...
<ui-input label="Password">
  ...
  protected readonly numberCode = `import { signal } from '@angular/core';
import { form, max, min } from '@angular/forms/signals';

readonly model = signal({quantity: 12});
readonly formState = form(this.model, path => {
  min(path.quantity, 1, {message: 'Quantity must be at least 1'});
  max(path.quantity, 100, {message: 'Quantity cannot exceed 100'});
});
...
readonly model = signal({email: 'not-an-email'});
readonly formState = form(this.model, path => {
  required(path.email, {message: 'Email is required'});
  email(path.email, {message: 'Use a valid email'});
});

<ui-input label="Work email" withErrorMessage>
  <input type="email" [formField]="formState.email" />
</ui-input>`;
  protected readonly disabledCode = `import { signal } from '@angular/core';
import { disabled, form } from '@angular/forms/signals';

readonly model = signal({identity: 'Managed by SSO'});
readonly formState = form(this.model, path => {
  disabled(path.identity, {when: 'Locked by identity provider'});
});
...
<ui-input label="Identity">
  ...
  protected readonly maskCode = `import { signal } from '@angular/core';
import { form } from '@angular/forms/signals';

readonly model = signal({phone: '998901234567', amount: '1250000.50'});
